...
messagebox.showinfo(title="Instructions", message="Set text filter on \"Removal Reason\" with inverse of \"REMOVE: SciTech Collection\" and the custom filter on \"Record Number\" to \"2\". For the remaining visible titles, determine if FSU has access and ?????????????") # See Ebook_Central_Custom_Faceting_pt8.json